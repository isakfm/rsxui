...
fn ButtonComponent(label: String, variant: String) -> String {
    let variant_class = if variant.is_empty() { "btn".to_string() } else { format!("btn btn-{}", variant) };
    rsx! {
...
async fn test_badge_variants() {
    for color in &["primary", "secondary", "accent", "neutral", "info", "success", "warning", "error"] {
        let html = rsx! {
...
async fn test_button_variants() {
    for variant in &["", "primary", "secondary", "ghost", "link"] {
        ...
        if variant.is_empty() {
            assert!(html.contains("class=\"btn\">"), "Button should have btn class: {}", html);
        } else {
            assert!(
